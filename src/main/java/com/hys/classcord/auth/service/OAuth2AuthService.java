...
public class OAuth2AuthService {
    ...
    public String handleOAuthLogin(AuthProvider provider, String token) {
        // 1. 根據平台動態取得對應的驗證策略
        OAuth2Strategy strategy = Optional.ofNullable(strategies.get(provider))
        ...
        UserIdentity identity = userIdentityRepository
                .findByProviderAndProviderUid(userInfo.provider(), userInfo.providerUserId())
                .orElseGet(() -> registerNewOAuthUser(userInfo)); // 沒來過就啟動自動註冊！

        // 4. 發放 Classcord 通行證
        return javaJwtUtils.generateToken(identity.getUser().getId(), identity.getUser().getEmail());
    }
    ...
    private UserIdentity registerNewOAuthUser(OAuthUserInfoDto userInfo) {
        ...
        if (existingUser.isPresent() && !userInfo.isEmailVerified()) {
            throw new AuthException(
                    ...
                    "資安保護：不允許使用未驗證該 Email 的第三方平台進行帳號融合"
            );
        }
        ...
        // 3. 建立新憑證紀錄並直接回傳
        return userIdentityRepository.save(UserIdentity.builder()
                .user(user)
