...
public class DefaultRagIndexingStrategy implements RagIndexingStrategy {
    ...
    @Override
    public void processAndIndex(InternalMaterialDto material, byte[] fileBytes) {
        log.info("開始使用 DEFAULT 策略解析教材 RAG: materialId={}", material.id());
        ...
        String serverId = material.serverId() != null ? material.serverId().toString() : "";
        String channelId = material.channelId() != null ? material.channelId().toString() : "";
        String materialId = material.id().toString();
        String fileName = material.originalName();
