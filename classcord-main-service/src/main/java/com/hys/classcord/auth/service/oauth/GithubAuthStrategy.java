    ...
    public OAuthUserInfoDto verifyAndExtractInfo(String code) {
        ...
        try {
            ...
            throw new AuthException(
                    ...
                    "連線至 GitHub 交換 Token 失敗: " + e.getMessage());
        }
        ...
        try {
            ...
            throw new AuthException(
                    AuthErrorCode.OAUTH_PROVIDER_ERROR, "調用 GitHub 用戶資訊接口失敗: " + e.getMessage());
        }
        ...
        if (email == null || email.isBlank()) {
            try {
                ...
                throw new AuthException(
                        AuthErrorCode.OAUTH_PROVIDER_ERROR, "嘗試撈取 GitHub 私密信箱時連線異常");
            }
        }
        ...
        if (email == null || email.isBlank()) {
            throw new AuthException(
                    AuthErrorCode.OAUTH_EMAIL_NOT_FOUND, "您的 GitHub 帳號未綁定或未公開已驗證的 Email");
        }

        return new OAuthUserInfoDto(
                githubUid, email, username, avatarUrl, AuthProvider.GITHUB, true);
    }
}
