...
public class MaterialService {
    ...
    /** 內部微服務專用：標記教材狀態為 PROCESSING (使用悲觀鎖防併發) */
    @Transactional
    public void markAsProcessing(UUID materialId) {
        Material material =
                materialRepository
        ...
        if (material.getStatus() == com.hys.classcord.material.enums.MaterialStatus.PROCESSING) {
            throw new MaterialException(MaterialErrorCode.AI_ASSISTANT_PROCESSING);
        }
        if (material.getStatus() == com.hys.classcord.material.enums.MaterialStatus.ENABLED) {
    ...
    /** 內部微服務專用：獲取教材簡要元數據與狀態 */
    public com.hys.classcord.common.dto.InternalMaterialDto getInternalMaterial(UUID materialId) {
        Material material =
                materialRepository
                        .findById(materialId)
                        .orElseThrow(
    ...
    /** 內部微服務專用：標記教材狀態為 ENABLED 並廣播通知前端 */
    @Transactional
    public void markAsEnabled(UUID materialId) {
        Material material =
                materialRepository
                        .findByIdForUpdate(materialId)
                        .orElseThrow(
    ...
    /** 內部微服務專用：標記教材狀態為 FAILED 並廣播通知前端 */
    @Transactional
    public void markAsFailed(UUID materialId, String errorMessage) {
