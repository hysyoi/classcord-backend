...
public class InternalQuizController {
    ...
    @Operation(
            summary = "標記 Quiz 出題 Job 完成並儲存題目",
            description = "供 AI 微服務完成出題後呼叫，委託 Service 入庫寫入題目並推送 SSE")
    public ResponseEntity<Void> completeJob(
            @PathVariable UUID jobId, @RequestBody SaveGeneratedQuestionsRequest request) {
    ...
    @Operation(
            summary = "標記 Quiz 出題 Job 為 FAILED",
            description = "供 AI 微服務出題失敗時呼叫，委託 Service 寫入失敗訊息並推送 SSE")
    public ResponseEntity<Void> markJobAsFailed(
        ...
        log.error(
                "【內部 API】收到 AI 服務回傳出題失敗通知: jobId={}, materialId={}, error={}",
                jobId,
                materialId,
                errorMessage);
        quizJobManager.updateJobStatusAndPushRequiresNew(
                jobId, materialId, JobStatus.FAILED, errorMessage);
        return ResponseEntity.ok().build();
    }
}
